...
inputPwCheck.addEventListener("input", (e) => {
  ...
  if (!pwSpan.classList.contains("check")){
    span.innerText = "유효한 형식의 비밀번호를 먼저 입력해 주세요";
    span.classList.add("error");
    span.classList.remove("check");
  ...
  if(val.length === 0){ // 입력된 값이 없을 경우
    span.innerText = "비밀번호를 먼저 입력해 주세요";
    span.classList.remove("check");
    span.classList.remove("error");
  ...
  if(pwVal == val){  // 비밀번호 / 비밀번호 확인이 같을경우
...
function checkPwFn() {
  ...
  if(checkVal.length === 0){ // 입력된 값이 없을 경우
    span.innerText = "비밀번호를 먼저 입력해 주세요";
    span.classList.remove("check");
    span.classList.remove("error");
...
signupForm.addEventListener("submit", (e) => {
  ...
  // HTML 요소가 가지고 있는 기본 이벤트를 막다(제ㄱ)
  /* 객체 전용 향상된 FOR문 : for(let key in 객체명) 
    -> 객체가 가지고있는 K를 순서대로 하나씩 key에 대입*/
