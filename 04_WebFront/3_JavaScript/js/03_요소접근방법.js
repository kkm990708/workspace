...
function idTest() {
  ...
  // Java에서 배열 리터럴 표기법 : {}
  // JS에서 배열 리터럴 표기법 : []
  const colorArray = ['red', 'yellow', 'blue', 'green', 'black', 'white'];

  // 배열명.length : 배열길이
  // Math.random() : 0.0 <= <1.0 난수(실수형)
...
/* class 로 접근하기 */
function classTest() {
  // 클래스 속석 값이 div2인 요소(객체)를 모두 얻어와 배열로 반환받기
...
function nameTest() {
  ...
  for (let i = 0; i < hobbyArray.length; i++) {
    ...
    /* 요소.checked : 해당요소가 체크되어 있으면 true, 아니면 false */
...
function readValue() {
  ...
  // "문자열".length : 문자열의 길이
  // "  문자열  ".trim() : 양쪽 공백 제거
  // 입력요소 값.trim().length == 0 : 아무것도 입력하지 않았다면
  if(input.value.trim().length == 0){
    ...
    // 요소.scrollTop = 높이 : 스크롤 높이를 해당 높이로 이동
  ...
  // input에 포커스 맞추기
  input.focus();  // 요소.focus() : 해당요소에 포커스 맞춤
}

// 아이디가 input인 요소에서
// 키보드 키가 올라올 때(kepup) 동작 (function(){})을 수행하라
