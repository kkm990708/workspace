...
SELECT EMP_ID ,EMP_NAME ,DEPT_CODE ,EMP_ID ,DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID;
...
SELECT EMP_NAME ,DEPT_TITLE
FROM EMPLOYEE e RIGHT JOIN DEPARTMENT d ON (DEPT_CODE = DEPT_ID);
...
--LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- LEFT JOIN(23행)
--RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- RIGHT JOIN(23행)
...
-- 사원의 급여가
-- SAL_LEVER에 작성된 최소(MIN_SAL) ~ 최대(MAX_SAL)
-- 범위의 급여가 맞을 때만 JOIN
---------------------------------------------------------------------------------------------------------------
...
-- TIP. 하나의 테이블만 생각하지 말고 똑같은 테이블이 두개가 있다고 생각
...
JOB_NAME 직급명,DEPT_TITLE 부서명,LOCAL_NAME 근무지역명, SALARY 급여 
FROM EMPLOYEE E 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) 
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
NATURAL JOIN JOB
...
-- 오라클 전용
SELECT EMP_ID 사번, EMP_NAME 이름,
J.JOB_NAME 직급명,DEPT_TITLE 부서명,LOCAL_NAME 근무지역명, SALARY 급여 
...
SELECT EMP_NAME, J.JOB_NAME, DEPT_TITLE, LOCAL_NAME
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME , SALARY,MIN_SAL, (SALARY * (1 + NVL(BONUS,0))) * 12 , S.SAL_LEVEL 
FROM EMPLOYEE E 
