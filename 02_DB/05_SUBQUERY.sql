...
FROM EMPLOYEE E 
JOIN JOB J USING(JOB_CODE)
JOIN DEPARTMENT D2 ON (DEPT_CODE = DEPT_ID)
...
-- 방법1 : SER OPERAIOM(집합 연산자) 중 UNION 사용
--3번
SELECT EMP_ID , EMP_NAME , DEPT_TITLE, JOB_NAME, '사수' "구분"
FROM EMPLOYEE E 
NATURAL JOIN JOB J 
...
AND SALARY > ANY(SELECT SALARY
			  ...
			  JOIN JOB J USING(JOB_CODE)
			  WHERE JOB_NAME = '과장');
-- A > ANY B : A 가 B의 여러 결과 중 하나라도 크면 TRUE 
			 --> A가 B의 최소값 보다 큰 경우
-- A < ANY B : A 가 B의 여러 결과 중 하나라도 작으면 TRUE 
			 --> A가 B의 최대값 보다 작은 경우
...
SELECT EMP_ID ,EMP_NAME ,DEPT_CODE ,MANAGER_ID ,EMP_NO ,HIRE_DATE 고용일
FROM EMPLOYEE E 
...
JOIN JOB USING(JOB_CODE)
WHERE HIRE_DATE =   ( -- 특정부서 입사일이 가장빠른사람
					SELECT MIN(HIRE_DATE)
					FROM EMPLOYEE SUB
					WHERE NVL(MAIN.DEPT_CODE,'XXX') = NVL(SUB.DEPT_CODE,'XXX')
					AND ENT_YN != 'Y' -- MAIN.DEPT_CODE가 NULLX
					)
ORDER BY HIRE_DATE ;
...
WHERE EXISTS 
	(SELECT EMP_ID 
	FROM EMPLOYEE SUB
	WHERE SUB.EMP_ID = MAIN.MANAGER_ID); --> 서브쿼리의 조회 결과가 있고, 없고가 중요
-- EXISTS (서브쿼리)
...
-- 모든 사원의 사원명, 급여, 전체사원 급여 평균
SELECT EMP_NAME, SALARY, (SELECT AVG(SALARY) FROM EMPLOYEE) "전체사원 급여 평균"
FROM EMPLOYEE E ;
...
SELECT EMP_ID, EMP_NAME, MANAGER_ID,
	NVL((SELECT EMP_NAME 
		 FROM EMPLOYEE SUB		
	 	 WHERE EMP_ID = MAIN.MANAGER_ID), '없음') "관리자명"
FROM EMPLOYEE MAIN;
...
SELECT * FROM 
	...
	FROM EMPLOYEE E 
	LEFT JOIN DEPARTMENT D ON (DEPT_CODE = DEPT_ID)
	--> 서브쿼리의 결과 RESULT SET이 다른 SELECT 문의 테이블 역할
	);
...
-- 급여를 많이 받는 순서로 조회 + 등수 포함
-- RANK() : 순위를 매기겠다 (공동 등수 후 그만큼 순위 증가)
-- OVER(ORDER BY SALARY DESC) : 급여 내림차 순으로
 ...
 * 	EX) 인라인 뷰에서 지정된 별칭을 메인 쿼리에서도 똑같이 사용
 * 
...
SELECT EMP_ID ,EMP_NAME ,JOB_NAME, 나이, 연봉
FROM(SELECT EMP_ID ,EMP_NAME ,JOB_NAME, JOB_CODE,
	FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,1,6) ,'RRMMDD')) / 365 ) 나이,
	TO_CHAR(SALARY * (1 + NVL(BONUS,0)) * 12,'L999,999,999') 연봉
	FROM EMPLOYEE E 
	JOIN JOB USING(JOB_CODE))
WHERE (JOB_CODE , 나이) IN (SELECT JOB_CODE, MIN(FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,1,6) ,'RRMMDD')) / 365 ))
			 FROM EMPLOYEE 
			 GROUP BY JOB_CODE)
ORDER BY 나이 DESC;
