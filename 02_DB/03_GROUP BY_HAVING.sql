...
/*3*/SELECT DEPT_CODE /*묶인 그룹의 이름*/,SUM(SALARY) /*그룹별 급여합*/ 
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DEPT_CODE -- DEPT_CODE가 같은행끼리 그룹으로 묶어라
...
--> (SELECT문 해석 순서를 잘 기억하는 것이 중요!!)


-- EMPLOYEE 테이블에서 부서코드가 'D5', 'D6'인 부서의 평균 급여 조회
SELECT DEPT_CODE , ROUND(AVG(SALARY)) 
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6') -- 부서 코드가 D5, D6인 행만 남음
GROUP BY DEPT_CODE ;
...
-- 부서별 급여평균이 3000000원 이상인 부서를 조회하여 부서코드 오름차순으로 정렬
SELECT DEPT_CODE, AVG(SALARY) 
FROM EMPLOYEE  
--WHERE SALARY >= 3000000	-- 급여가 300만 이상인, 사원(행만조)
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >= 100000
-- 묶여진 그룹의 급여 평균이 300만 이상인 경우 조회 결과에 포함
ORDER BY DEPT_CODE ;


-- 부서별 그룹의 급여 합계 중 9백만원을 초과하는 부서코드와 
-- 급여 합계 조회
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE , SUM(9000000) 
FROM EMPLOYEE 
GROUP BY DEPT_CODE 
