...
SELECT DEPARTMENT_NAME 학과이믈, STUDENT_NAME 학생이름, PROFESSOR_NAME 지도교수
FROM TB_STUDENT TS 
JOIN TB_DEPARTMENT TD USING (DEPARTMENT_NO)
...
FROM TB_PROFESSOR TP 
RIGHT JOIN TB_STUDENT TS ON(PROFESSOR_NO = COACH_PROFESSOR_NO)
...
SELECT STUDENT_NO 학번, STUDENT_NAME 학과, DEPARTMENT_NAME 학과이름, AVG(POINT)
FROM TB_STUDENT TS 
JOIN TB_GRADE TG USING(STUDENT_NO)
JOIN TB_DEPARTMENT TD USING(DEPARTMENT_NO)
WHERE ABSENCE_YN = 'N'
...
WITH KOREAN_AVG AS (SELECT STUDENT_NO ,STUDENT_NAME, AVG(POINT) 평균
					FROM TB_STUDENT TS
					JOIN TB_GRADE TG USING (STUDENT_NO)
					WHERE DEPARTMENT_NO = (
						SELECT DEPARTMENT_NO 
