...
-- 컬럼명 자료형
	...
	MEMEBER_SSN CHAR(14),
	ENROLL_DATE DATE DEFAULT SYSDATE -- 기본값을 현재시간으로
);
...
COMMENT ON COLUMN MEMBER.MEMEBER_SSN IS '회원 등록 번호';
...
('MEM01', '123ABC', '홍길동', '990123-1234567', DEFAULT);
...
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME, MEMEBER_SSN)
...
/*
    ...
    FOREIGN KEY (다른 테이블의 기본기 또는 UNIQUE가 설정됨 컬럼을 참조)
    	-> 다른 테이블의 값을 가져다 씀
...
--    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE , -- 컬럼 레벨 제약조건 
    							-- CONSTRAINT 제약조건명, 제약조건종류
    USER_ID VARCHAR2(20), -- 컬럼레벨 제약 조건 X
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    PHONE VARCHAR2(30),
...
--    UNIQUE(USER_ID) -- 제약조건(컬럼명) -> 제약조건명 미 지정
...
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(A230724_KMS_KANG.SYS_C0013070)에 위배됩니다
-- SYS_C0013070 : 제약조건 이름 미설정시 지정되는 이름
INSERT INTO USER_USED_UK
...
CREATE TABLE USER_USED_PK(
    -- 컬럼 레벨로 PK 지정
--    USER_NO NUMBER PRIMARY KEY, -- 컬럼레벨, 이름 지정 X
--    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼레벨, 이름 지정 O
    USER_NO NUMBER,
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
--  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK1 REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 레벨
  GRADE_CODE NUMBER,
...
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 ...
 -- SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(A230724_KMS_KANG.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
-- 참조하는 테이블 = 자식 테이브
-- 참조를 당하는 테이블 = 부모 테이블
...
-- USER_USED_UK , USER_GRADE 테이블 JOIN
...
-- USER_USED_FK 테이블에서 참조중인 값(GRADE_CODE = 10) 삭제
DELETE FROM USER_GRADE 
...
CREATE TABLE USER_USED_FK3(
  ...
  REFERENCES USER_GRADE3 -- 참조할 컬럼을 작성하지 않으면 PK 자동 참조 
  ON DELETE CASCADE   
);
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR2(15) CONSTRAINT NN_PNO NOT NULL,
	GENDER VARCHAR2(10) CONSTRAINT CK_GENDER CHECK(GENDER IN('남','여')),
	-- GENDER NVARCHAR(1) -- 유니코드 글자수 1개
	PHONE VARCHAR2(20),
	ADDRESS VARCHAR2(60),
...
VALUES(1, 'user01', 'pass01', '111111-1234567', '남', '010-1234-5678', '1번지', 'N');

INSERT INTO USER_TEST 
VALUES(2, 'user02', 'pass02', '211111-1234567', '여', '010-2345-1111', '2번지', 'N');

INSERT INTO USER_TEST 
VALUES(3, 'user03', 'pass03', '311111-1234567', '남', '010-2233-2222', '3번지', 'Y');

INSERT INTO USER_TEST 
VALUES(4, 'user04', 'pass04', '411111-1234567', '여', '010-4444-2222', '4번지', 'Y');

INSERT INTO USER_TEST 
VALUES(5, 'user05', 'pass05', '541111-1234567', '여', '010-5555-2222', '5번지', 'N');
...
ALTER TABLE EMPLOYEE ADD
CONSTRAINT SAL_GRADE_FX 
FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE;
-- FX제약조건 추가 (EMPLOYEE.SAL_LEVER 컬럼이 SAL_GRADE의 PK 참조)
