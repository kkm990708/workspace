-- 함수 : 컬럼값 | 지정된값을 읽어 연선한 결과를 반환하는 것

-- 단일행 함수 : N개의 행의 컬럼 값을 전달하여 N개의 결과가 반환

-- 그룹 함수 : N개의 행의 컬럼 값을 전달하여 1개의 결과가 반환
--			(그룹의 수가 늘어나면 그룹의 수 만큼 결과가 반환)

-- 함수는 SELECT, WHERE, ORDER BY, GROUP BY, HIVING 절에서 사용가능
...
-- ROUND(숫자 | 컬럼명 [, 소수점위치])
-- 소수점 위치 지정 X : 소수점 첫째 자리에서 반올림 -> 정수표현
-- 소수점 위치 지정 O 
...
-- SYSDATE : 현재 시간
-- SYSTIMESTAMP : 현재시간(MS 포함, 표준시 대사)
SELECT SYSDATE , SYSTIMESTAMP FROM DUAL;


-- MONTH_BETWEEN(날짜, 날짜): 두날자 사이의 개체수를 반환
SELECT MONTHS_BETWEEN ('2023-12-26', SYSDATE)
FROM DUAL;

-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 조회수
SELECT EMP_ID, EMP_NAME, HIRE_DATE,
	MONTHS_BETWEEN(SYSDATE, HIRE_DATE) / 12 "N차"
FROM EMPLOYEE;


 -- 날짜 + 숫자 : 일 단위로연산 ( 윤년 계산이 안됨)
-- MONTHS_BETWEEN() : 윤년계산이 포함되어 있음

-- ADD_MONTHS(날짜, 숫자) : 날짜를 숫자만큼의 개월수를 더하여 반환
SELECT SYSDATE , ADD_MONTHS(SYSDATE,1) "한달후", SYSDATE, ADD_MONTHS(SYSDATE, -1)  FROM DUAL;


--LAST_DAY(날짜) : 해당월의 마지막 일을 받환
...
SELECT 1234,TO_CHAR(1234, '99999999999') FROM DUAL; 

-- 0: 숫자 한 칸을 의미, 오른쪽 정렬, 빈 칸에 0을 추가
SELECT 1234,TO_CHAR(1234,'0000000000')FROM DUAL; 

-- 숫자의 칸 수보다 패턴의 칸 수가 작을 때
--> 값이 제대로 표현될 수 없어서 #####이 반환됨
SELECT 123456789, TO_CHAR(123456789,'000') FRODBA_SCHEDULER_INCOMPATS DUAL;

SELECT TO_CHAR(1234567),
		TO_CHAR(1234567, 'L999999999')
FROM DUAL;
...
-- 2023년 8월 22일 화요일 오후 2시 35분 22초
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" DAY PM HH"시" MI"분" SS"초"') 
FROM DUAL;
-- ORA-01821 날짜 형식이 부적합합니다
-- 년,월,일,시,분,초 는 오라클에서 지정한시간 패턴이 아니다
	--> 패턴인식이 아닌있는 그대로 출력함을 나타내는 ""(쌍따옴표) 사용
...
/********** 연도 패턴 Y, R 의 차이점**********/
-- 연도 가 두자리만 작성되어 있는 경우 
-- 50 미만 : Y,R 둘다 누란된 연도 앞부분에 현재 세기(2000년대) 추가
...
SELECT EMP_NAME, SALARY * 12 "연봉", 
		NVL(BONUS * SALARY * 12, 0) "인센티브" 
FROM EMPLOYEE
ORDER BY "연봉" DESC ;

----------------------------------------

-- NVL2(컬럼명, NULL이 아닌경우 변경할 값 , NULL인 경우 변경할 값)
...
-- 모든 조건을 만족하지 못하면 [,아무것도 만족X] 가 반환됨
...
SELECT EMP_NAME ,JOB_CODE ,SALARY 기존급여,
	DECODE(JOB_CODE, 'J7', SALARY * 1.1,
					 'J6', SALARY * 1.15,
					 'J5', SALARY * 1.2, SALARY * 1.05) 지급급여
FROM EMPLOYEE;
...
SELECT MAX(HIRE_DATE), MIN(HIRE_DATE), MAX(EMP_NAME), MIN(EMP_NAME) 
FROM EMPLOYEE;
...
-- COUNT(컬럼명) : 지정된 컬럼 값이 NULL이 아인 행의 개수를 반환
-- 				(NULL인 행 미포함)

-- COUNT(DISTINCT 컬럼명) :
	-- 지정된 컬럼에서 중복 값을 지외한 행의 개수를 반환
...
-- EMPLOYEE 테이블에 존재하는 부서코드의 수르 조회
...
SELECT COUNT(DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남자', NULL)) ,
COUNT(DECODE(SUBSTR(EMP_NO, 8, 1), 2, '여자', NULL)) 
FROM EMPLOYEE;
