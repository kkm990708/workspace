...
-- VALUES ()
SELECT *
FROM EMPLOYEE2 ORDER BY EMP_ID DESC;
...
DELETE FROM EMPLOYEE2 E WHERE EMP_ID = 900;
COMMIT;
...
FROM EMPLOYEE2 WHERE EMP_NAME IN('유재식','방명수');
...
UPDATE EMPLOYEE2 SET (SALARY, BONUS) = (SELECT SALARY, BONUS FROM EMPLOYEE E WHERE EMP_NAME = '유재식')
...
SELECT * FROM EMP_M01; 
SELECT * FROM EMP_M02; -- M01 과 겹치는 4행 겹치지 않는 1행
...
WHEN NOT MATCHED THEN -- 같은 사번이 존재하지 않으면
...
ROLLBACK; -- DML(INSERTM UPDATE, DELETE) 실행취소
