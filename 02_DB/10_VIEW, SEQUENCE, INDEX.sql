...
-- 지역명에 ASIA 가 포함되는 직원 사번 내림차순 조회
SELECT * FROM V_EMP
WHERE 지역명 LIKE '%ASIA%'
ORDER BY 사번 DESC;
...
-- INSERT 확인
SELECT * FROM V_DCOPY2 VD ;

-- VIEW 생성에 사용한 원본 테이블(DEPT_COPY2) 확인
SELECT * FROM DEPT_COPY2 DC ;
-- VIEW 에 삽입을 시도했는데 원본 테이블에 삽입
-- VIEW 는 개별적인 저장 공간이 있는 테이블이 아니라
-- 원본 테이블의 필요한 컬럼, 행을 모아서 보여주는 가상의 테이블
--> NULL 이 삽입 == 무결성을 약하게(위배) 만드는 원인
ROLLBACK;
...
INCREMENT BY 5	-- NEXTVAL 마다 5 씩 증가
MAXVALUE 150    -- 증가 가능한 최대값 150
NOMINVALUE		-- 최소값 없음
NOCYCLE			-- NO CYCLE 최대값 까지 증가후 반복X
NOCACHE 		-- 미리 발생시킬 숫자를 만들어 않음
;

-- 현재 시퀸스 값 확인
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL;
-- 먼저 NEXTVAL 수행후 CURRVAL 수행

-- 다시 NEXTVAL 호출
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;

-- TB_TEST에 INSERT시 시퀸스 확인하기
...
/* 기본적으로 CREATE 작성법과 같으나 START WITH 는 제외
 * 
 [작성법]
...
-- 시퀸스 START WITH를 다시 지정하고 싶으면 삭제후 다시 만들어야한다
