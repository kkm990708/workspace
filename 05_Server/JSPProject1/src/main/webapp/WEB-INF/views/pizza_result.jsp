...
<body>
  ...
  <pre>
    JSP의 기본 표현식 <%-- <%= %> --%>
    조금 더 효율적이고 간단히 자동으로 작성하 수 있도록 고안된 언어
    (JSP) 기본 내장

    화면에 출력하고자 하는 Java코드(값, 변수명 등)를
    \${ KEY} 형식으로 작성하면
    KEY에 해당하는 vlaue가 화면에 표시됨
     </pre>

     <hr>
    <h3>\$param 속성값 : 파라미터 출력하기 </h3>
    <ul>
      <li>${param.pizza}</li>
      <li>${param.size}</li>
      <li>${param.amount}</li>
  ...
  price = <%= (int)( request.getAttribute("price")) %> <br>
  ...
  Java 코드(if, for, 변수 선언, 형변환 등)

  JSP에서 작성하기 편하게 태그를 만들어
  표준으로 제공하는 라이브러리
  (만들어진 태그 == 커스텀 태그)
  </pre>
