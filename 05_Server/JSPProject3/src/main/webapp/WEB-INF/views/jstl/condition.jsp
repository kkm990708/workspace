...
<body>
  ...
  <h1>2. 연속된 조건문 (c:choose / c:when / c:otherwise 태그) </h1>
  <%-- if ~ else if ~ else --%>

  <pre>
  c:choose 태그 : c:when / c:otherwise를 감싸는 태그
  (이안에 if ~ else if ~ else 구문을 쓰겠다)
  ...
  *** c:choose 태그 작성 주의 사항 ***
  ...
  - c:when(언제) 태그 : if,else if 를 나타내는 태그
    속성은 test밖에 없음 (c:if 참고)
