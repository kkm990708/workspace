...
public class MapEx {
	...
	// K:V 한쌍으로 데이터 저장(K:V 한쌍을 묶어서 Entry 라고 부름)
	
	// - key : 키워드 역할
	// 1) 중복 X (key 중복시 value를 새로운 내용으로 덮어 씌움)
	...
	// 1) key로 인해서 구분됨 -> 중복 허용 -> Value만 묶어놓으면 List와 같은 특징을 지님
	
	public void ex1() {
		// key 제한, value 제한
		Map<Integer, String> map = new HashMap<Integer, String>(); 
		// 1. V map.put(K k, V v) : Map에 추가
		// Map 에 추가 시 Key가 중복되면 새로운 Value로 덮어 씌우고 이전 Value가 반환된다
		// 중복 아니면 null 반환
		...
		// 2. V map.get(K k) : map에서 일치하는 Key가 업으면 null반환
		System.out.println();
		System.out.println(map.get(1));
		System.out.println(map.get(2));
		System.out.println(map.get(3));
		// value 가 String 이니까 String 변수에 저장
		String s = map.get(1);
		
		// 3. int mpa.size : 저장된 K:V쌍의 개수 반환
		System.out.println("K:V 몇쌍 : " + map.size() + "쌍");
		
		// 4. V map.remove(K k) : map에서 Key가 일치하는 요소(K:V 쌍)을 제거
		// 제거되는 V 변환 없으면 null반환
		System.out.println(map.remove(1) + "제거");
		System.out.println(map.remove(9) + "제거");
		System.out.println(map.remove(1) + "제거");
		
		// 5. void map.clear() : map 다지움
		map.clear();
	...
	public void ex2() {
		...
		// 1. Set map.keySet() : key만을 모아놓은 집합(Set) 을 반환
		...
		// 2. keySet을 이용해서 Map 에 있는 모든 요소 순차 접근
		for( String key : set) {
