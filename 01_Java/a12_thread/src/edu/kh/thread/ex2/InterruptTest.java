...
public class InterruptTest{
	/* interrupt : 일시정지, 방해하다
	 		-> 스레드의 동작을 방해해서 멈추게 함
	 		void interrupt()
	 		- 현재 스레드의 interrupted 필드값을 true 로 변경
			interrupted  필드 == false -> 작업 진행
			interrupted  필드 == true -> 작업 멈춤

			단 Thread.sleep()에 의해서 일시 정지된 스레드에
			interrupt()를 호출하게 되면 스레드의 interrupted 필드값이
			true로 변하지않고(작업종료 X) InterruptedException 이 발생한다
	...
	*/ 
	public void test() {
		...
		// ----------------------
		// 엔터 입력 시 sleep2 스레드를 멈추가하는 interrupt() 호출
		sleep2.interrupt();
