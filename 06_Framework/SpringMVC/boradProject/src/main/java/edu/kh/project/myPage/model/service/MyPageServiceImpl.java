...
// Service : 비즈니스 로직 처리(데이터가공, DB연결, 트랜젝션 제어)
...
public class MyPageServiceImpl implements MyPageService{
	...
	@Autowired // MyPageDAO(또는 상속관계) bean DI (의존성 주입)
	...
	public int changePw(String currentPw, String newPw, int memberNo) {
		...
		// 2. 현재 비밀번호와 조회환 비밀번호가 같은지 확인
		// 같으면 -> 비밀번호 변경
		// 다르면 -> return 0
		
		// BCrypt에서 제공하는 matches() 이용
		if (!bcrypt.matches(currentPw, encPw)) {
			// 현재 비밀번호와 조회한 비밀번호가 다른경우
			return 0;
		}

		// 3. 비밀번호 변경 DAO 메서드 호출 전
		// newPw, memberNo를 하나의 객체에 저장
		// 마이바티스 코드는 파라미터를 하나만 전달 할 수 있어서
