...
memberNickname.addEventListener("input", () => {
    ...
    if(regEx.test(memberNickname.value) ){
        ...
        fetch("/member/checkEmail?email="+memberNickname.value)
        .then( response => response.text() )
        .then( result => {
...
memberTel.addEventListener("input", () => {
    ...
    if(regEx.test(memberTel.value) ){
        ...
        telMessage.classList.remove("error"); // 빨간글씨 제거
    ...
    // 입력 받은 이메일이 정규식과 일치하지 않는경우
        ...
        telMessage.innerText = "알맞은 전화번호 형식으로 작성해주세요"
        telMessage.classList.add("error"); // 초록색 글씨
        telMessage.classList.remove("confirm"); // 빨간글씨 제거
        checkObj.memberTel = false;
    }
