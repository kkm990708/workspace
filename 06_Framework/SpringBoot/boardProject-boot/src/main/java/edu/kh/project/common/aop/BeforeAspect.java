...
@Slf4j
@Component // bean 으로 등록 => Spring이 알아서 다룰 수 있음
@Aspect	// 공통 관심사가 작성된 클래스임을 명시
		// 공통 관심사 : 특정 지점에서 동작하는 코드
public class BeforeAspect {
	
	// @Before(포인트컷) : 포인트컷 지점 코드 전에 수행
	//  execution( [접근제한자(생략가능)] 리턴타입  클래스명   메소드명 ([파라미터]) )
	// edu.kh.project 패키지 내부에
	// Controller 라고 하는 이름이 포함된 클래스의 
	// 모든 메서드( 반환형, 매개변수 상관 X)를 지정
	...
	public void beforeLog(JoinPoint jp) {
		...
		log.info(className + "." + methodName +  "() 요청됨");
