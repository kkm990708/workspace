...
/** 페이징 처리에 필요한 모든값을 계산하고 저장하는 객체<br>
 * 현재 페이지, 전체 게시글 수 ,
 * 한 페이지에 보여질 게시글 수,
 * 보여질 페이지 번호 개수,
 * 보여지는 페이지 번호 중 시작 번호,
...
public class Pagination {
   ...
   public Pagination(int currentPage, int listCount) {
	   ...
	   clacPagination(); // 객체 생성 시 계산 수행해서 모든 필드 초괴화
   }
...
/* listCount, currentPage, limt, pageSize 변경 시 다시 계산 필여*/
...
/* 필드에 들어갈 값 계산하는 메서드 */
   private void clacPagination() {
	   // maxPage : 마지막 페이지 번호 == 총 페이지 수
	   // 한 페이지에 게시글이 10개씩 보여질 경우
	   // 전체 게시글 수 : 100개 -> 페이지 : 10개
	   // 전체 게시글 수 : 91개 -> 페이지 : 10개
	   // 전체 게시글 수 : 101개 -> 페이지 : 11개
	   
	   maxPage = (int)Math.ceil( (double)listCount / limit);
	   
	   //(int)Math.ceil( (double)  / limit)
